...
export default function AuthorPage() {
  ...
  useEffect(() => {
    const fetchPostsByAuthor = async () => {
      try {
        ...
        const filtered = res.data.filter(post => post.author?.userName === author);
        setPosts(filtered);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <main style={{ flex: 1, display: "flex", flexDirection: "column", padding: "2rem", gap: "1rem" }}>
        <h2 style={{ color: themeColors.text }}>Posts by {author}</h2>
        {loading ? (
